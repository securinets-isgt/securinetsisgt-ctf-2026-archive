...
import { skillBreakdownMixin } from "./skill-breakdown";

const TEAM_PROFILE_UPDATE_INTERVAL = window.teamProfileUpdateInterval || 60000;
...
export function createTeamGraphs({ teamId, teamName }) {
  const context = window.TEAM_CONTEXT || {};
  const resolvedTeamId = Number(teamId || context.id || 0);
  const resolvedTeamName = teamName || context.name || "Team";
  ...
  return {
    ...
    memberCount: Number(context.member_count || 0),
    liveSyncedAt: null,
    loading: true,
    pollingHandle: null,
    ...skillBreakdownMixin,
    ...
    getLastSolveLabel() {
      const solve = this.getLastSolve();
      return solve?.challenge?.name || "No solves yet";
    },
    ...
    async loadStanding() {
      ...
      try {
        ...
        const standing = this.getTeamStanding();
        ...
        console.warn("Unable to refresh team standing", error);
      }
    },
    ...
    async refreshData() {
      ...
      try {
        const [solves, fails, awards] = await Promise.all([
          CTFd.pages.teams.teamSolves(resolvedTeamId),
          CTFd.pages.teams.teamFails(resolvedTeamId),
          CTFd.pages.teams.teamAwards(resolvedTeamId),
        ]);
