...
INSTANCE_TTL_SECONDS = 7200
LOCAL_HOST_ALIASES = {"localhost", "127.0.0.1", "0.0.0.0", "host.docker.internal", "::1"}
...
def get_tracker_query(session_obj, challenge_name=None):
    if is_teams_mode():
        query = DockerChallengeTracker.query.filter_by(team_id=session_obj.id)
    else:
        query = DockerChallengeTracker.query.filter_by(user_id=session_obj.id)
    if challenge_name is not None:
        query = query.filter_by(challenge=challenge_name)
    return query


def slugify_container_name(value):
    value = re.sub(r"[^a-zA-Z0-9_.-]+", "-", str(value or "").strip().lower())
    value = value.strip("-._")
    return value or "challenge"


def strip_host_port(value):
    if not value:
        return ""
...
def get_player_visible_host(docker, request_host=None):
    configured_host = strip_host_port(normalize_docker_hostname(docker.hostname if docker else ""))
    request_visible_host = strip_host_port(request_host or "")

    if configured_host.lower() in LOCAL_HOST_ALIASES:
        return request_visible_host or "localhost"

    return configured_host or request_visible_host or "localhost"
...
def do_request(docker, url, headers=None, method='GET'):
    ...
    host = normalize_docker_hostname(docker.hostname)
...
def get_client_cert(docker):
    ...
    try:
        ...
        ca_file.write(ca if isinstance(ca, bytes) else ca.encode())
        ca_file.seek(0)
        client_file = tempfile.NamedTemporaryFile(delete=False)
        client_file.write(client if isinstance(client, bytes) else client.encode())
        client_file.seek(0)
        key_file = tempfile.NamedTemporaryFile(delete=False)
        key_file.write(ckey if isinstance(ckey, bytes) else ckey.encode())
        key_file.seek(0)
...
def get_repositories(docker, tags=False, repos=False):
    ...
    result = list()
    items = r if isinstance(r, list) else r.json()
    for i in items:
        if not i['RepoTags'] == []:
...
def create_container(docker, image, team, portbl, challenge_name=None):
    tls = docker.tls_enabled
    CERT = None
    if not tls:
        prefix = 'http'
    ...
    challenge_slug = slugify_container_name(challenge_name or image.split(':')[0])
    image_tag = slugify_container_name(image.split(':')[-1] if ':' in image else image)
    container_name = f"{challenge_slug}-{image_tag}-{team}"
    assigned_ports = dict()
    ...
    if tls:
        ...
        result = r.json()
        if 'Id' not in result:
            return {"success": False, "message": result.get("message", "Docker failed to create the container.")}, data
...
class DockerChallengeType(BaseChallenge):
    ...
    def solve(user, team, challenge, request):
        ...
        docker = DockerConfig.query.filter_by(id=1).first()
        try:
            session_obj = team if is_teams_mode() else user
            docker_containers = get_tracker_query(session_obj, challenge.name).first()
...
class ContainerAPI(Resource):
    ...
    def get(self):
        ...
        if is_teams_mode():
            ...
            for i in containers:
                if int(session.id) == int(i.team_id) and (unix_time(datetime.utcnow()) - int(i.timestamp)) >= INSTANCE_TTL_SECONDS:
            ...
            for i in containers:
                if int(session.id) == int(i.user_id) and (unix_time(datetime.utcnow()) - int(i.timestamp)) >= INSTANCE_TTL_SECONDS:
                    delete_container(docker, i.instance_id)
                    DockerChallengeTracker.query.filter_by(instance_id=i.instance_id).delete()
                    db.session.commit()
        check = get_tracker_query(session, challenge).first()
        
        # If this container is already created, we don't need another one.
        if check != None and not (unix_time(datetime.utcnow()) - int(check.timestamp)) >= ACTION_COOLDOWN_SECONDS:
            return abort(403, f"To prevent abuse, instances can be restarted or stopped after {ACTION_COOLDOWN_SECONDS // 60} minute of creation.")
        # Delete when requested
        elif check != None and request.args.get('stopcontainer'):
            delete_container(docker, check.instance_id)
            get_tracker_query(session, challenge).delete()
            db.session.commit()
        ...
        create = create_container(docker, container, session.name, portsbl, challenge_name=challenge)
        if 'Id' not in create[0]:
            return abort(403, create[0].get('message', 'Docker failed to create the container.'))
        ports = json.loads(create[1])['HostConfig']['PortBindings'].values()
        created_at = unix_time(datetime.utcnow())
        entry = DockerChallengeTracker(
            team_id=session.id if is_teams_mode() else None,
            user_id=session.id if not is_teams_mode() else None,
            docker_image=container,
            timestamp=created_at,
            revert_time=created_at + ACTION_COOLDOWN_SECONDS,
            instance_id=create[0]['Id'],
            ports=','.join([p[0]['HostPort'] for p in ports]),
            host=get_player_visible_host(docker, request.host),
            challenge=challenge
        )
...
class DockerStatus(Resource):
    ...
    def get(self):
        ...
        data = list()
        for i in tracker:
            visible_host = get_player_visible_host(docker, request.host)
            data.append({
                'id': i.id,
                'team_id': i.team_id,
                'user_id': i.user_id,
                ...
                'expires_at': i.timestamp + INSTANCE_TTL_SECONDS,
